...
Legal constraints map directly onto $\mathbf{C}$. GDPR data minimization becomes a preference for observing low biometric exposure; EASA geofencing becomes an aversion to GPS readings inside restricted zones; EU AI Act traceability becomes a preference for maintaining an audit trail. We make $\mathbf{C}$ depend on \textit{inferred} context. Urgency and privacy status are hidden state factors that the agent must estimate from noisy observations. When the agent infers that an emergency is occurring, preference magnitudes shift: the drive toward the mission target rises from $-2$ to $+6$, the direct aversion to occupying the privacy zone relaxes from $-6$ to $0$, and the complaint aversion that normally prevents crossing falls from $C_3 = [8, -8]$ to $[1, -1]$ (Section~\ref{sec:experiments} gives the full profiles). The combined EFE computation across all observation modalities then favors advancing. No explicit rule switching is needed. The priority ordering lives in the relative sizes of preference values.
...
\textsc{pragmatic\_only} is statistically indistinguishable from \textsc{full} on C1 and C5 (violation $0.10$ vs.~$0.10$ and $0.08$ vs.~$0.07$; Mann--Whitney $p > 0.9$ on both): preference-driven policy selection produces the compliance behavior. On C7 the full agent is measurably cleaner: both reach the target on every trial, but removing the epistemic term raises the violation rate from $0.08$ to $0.13$ ($p = 0.035$, $d = 0.21$), so information-seeking contributes restraint precisely where the timing of the context switch matters. \textsc{epistemic\_only} fails completely (success $0.00$, violation $1.00$ on all three conditions): without preferences, the agent treats the privacy zone as a high-information region and lingers there, accumulating violations while it gathers evidence about the hidden state. So the epistemic term does not drive behavior on its own; in the full agent it acts as a regularizer that discourages commitment before the legal context is inferred. The near-equivalence on C1 and C5 has a mechanistic reading: caution under context uncertainty is already delivered by the mixture itself, since an unresolved posterior blends the profiles toward hesitant policies. The epistemic term adds drive only where observation precision is position-dependent, and here the privacy cue is legible only near the zone, so approach-to-sense behavior is nearly free and removing the term costs little; C7, where the violations it prevents are concentrated, is exactly the condition where sensing before committing pays. Figure~\ref{fig:ablation} shows the same data plus the time the agent spends at APPROACH before advancing: \textsc{full} and \textsc{pragmatic} both wait ${\sim}6.6$--$6.8$ timesteps before committing on C7; \textsc{epistemic} commits in $1$ timestep (straight into the privacy zone for information gain); \textsc{random} averages $\sim 2$.
...
The C7 numbers are where the agents differ. \textsc{hpm\_oracle}, even given the true context, reaches the target $1.00$ of the time and violates privacy $1.00$ of the time: a rule that knows the context still crosses while privacy is active, because it does not condition on \textit{when} the emergency authorization arrived. \textsc{hpm\_noisy} and \textsc{bayes\_rules} both violate at $0.79$--$1.00$ on C7. AIF reaches the target on every trial with $0.19$ violation, a region of the success--violation plane that none of the baselines reach. On C5 (privacy suspends at $t{=}7$, no emergency), \textsc{hpm\_oracle} still has the edge: it achieves $1.0$ success / $0$ violation by waiting until $t{=}7$ and crossing then, while AIF's belief-driven timing is imperfect ($0.93$ success, $0.13$ violation). So AIF is not uniformly best across the seven conditions; it is uniquely correct on C7, the condition where the legal trigger (emergency at $t{=}4$) and the privacy constraint (active until $t{=}7$) are out of phase. The C7 separation is structural rather than a matter of tuning. During $t = 4$--$6$, C7 presents the same context state as C2, (EMERGENCY, ACTIVE), so any memoryless rule on context, even the true context, must treat the two alike: cross (violating C7's still-active constraint early) or refuse (failing C2's mandated override). Distinguishing them requires integrating evidence over time, which is what the belief-weighted mixture does: in C2 the agent has accumulated enough confidence in the emergency to cross by $t \approx 7$, while in C7 privacy suspends at $t = 7$ before three timesteps of emergency evidence have shifted $C_{\text{eff}}$ far enough, and only a minority of trials cross earlier ($0.06$ in the Section~5.3 battery, $0.19$ under this experiment's seeds). Belief-weighted profile mixing also produces graded behavior across the four context combinations (Figure~\ref{fig:baselines}).
...
\textsc{full\_learn} produces $1.00/1.00$ on every privacy-active condition (C1, C5, C7), behavior indistinguishable from \textsc{misspecified}. The failure is not a lack of fit in any aggregate sense: after training, the learned emergency-signal matrix sits within $L^1 = 0.06$ of the truth, yet C7 violation is $1.00$. A model that is close in norm can still be behaviorally wrong.
...
Encoding legal obligations as prior preferences in an Active Inference agent lets a drone handle conflicts between GDPR, EASA, and EU AI Act requirements that rule-based systems cannot resolve. Unlike defeasible-logic frameworks that take legal context as a known input, the agent infers context from noisy observations and seeks information before acting.

Pragmatic-value optimization produces the compliance behavior; the epistemic term keeps the agent from acting before context is inferred. On C7 (emergency authorization arriving while privacy is still active), AIF is the only one of four agents tested that reaches the target on every trial while keeping violations low; on the simpler conditions, a rule-based hierarchical predictive model given the true context can do as well or better, and where a single obligation applies the AIF agent does no better than the hand-coded rule. The framework tolerates roughly an order of magnitude of misspecification in the preference magnitudes. Under sensor noise it largely stops acting, which keeps it from violating.

The clear limit is in the learning regime: a Bayesian agent that tries to acquire its full observation model from data loses the discrimination needed for emergency override, and calibrating the compliance-relevant channels recovers most but not all of it, since the sanction channel is one that no compliant agent can learn from experience. We expect the approach to generalize to any autonomous system that must balance competing legal obligations at runtime under partial observability, with the same condition on which parts of the generative model are calibrated versus learned.
